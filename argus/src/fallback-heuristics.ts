...
// Pure-algorithm replacements for Gemini functions.
// No API calls — regex/pattern matching only.
// Used when Gemini is down or rate-limited.

import type { GeminiExtraction, GeminiValidation, Event } from './types.js';
import type { ActionResult, ChatResponse, PopupBlueprint } from './gemini.js';

// ============ Date helpers (offline, no Gemini) ============
...
export function heuristicAnalyze(
  ...
  let eventType: GeminiExtraction['events'][0]['type'] = 'other';
  let title = '';
...
export function heuristicChat(
  ...
  if (scored.length === 0) {
    ...
    return {
      response: "I couldn't find any matching events. (AI is in fallback mode — try again when Gemini is back online for smarter search.)",
      relevantEventIds: [],
    };
  }
...
// Simply delegates to getDefaultPopupBlueprint from gemini.ts
...
export function heuristicGeneratePopupBlueprint(
  ...
  // Re-implemented here to avoid circular import with gemini.ts
    ...
    case 'context_reminder':
      if (event.event_type === 'recommendation') {
        return {
          ...
          body: `${sender !== 'Someone' ? sender + ' mentioned' : 'You mentioned'}: "${event.title}". Check it out!`,
